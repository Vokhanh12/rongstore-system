class Validators {
  ...
  String? validateCommon(String? textCommon) {
    if (textCommon!.isEmpty || textCommon == "Nhấn chọn (❌)") {
      return 'Dữ liệu bắt buộc nhập';
    }

    return null;
  }

  String? validateQuantityCommon(String? textCommon, {int maxValue = 1000}) {
    if (textCommon == null || textCommon.isEmpty) {
      return 'Dữ liệu bắt buộc nhập';
    }

    // Kiểm tra nếu có số 0 ở đầu
    if (textCommon.length > 1 && textCommon.startsWith('0')) {
      return 'Không được nhập số có số 0 ở đầu';
    }

    final intValue = int.tryParse(textCommon);
    if (intValue == null) {
    ...
    if (intValue <= 0) {
      ...
      return 'Giá trị không được lớn hơn $maxValue';
    }

    return null; // Trả về null nếu không có lỗi
  }

  String? validatePalletProduct(String? textCommon, String? textProduct) {
    if (textCommon!.isEmpty && textProduct!.isEmpty) {
      return 'Vui lòng chọn hàng hóa trước ';
    } else if (textCommon.isEmpty) {
      return 'Dữ liệu bắt buộc nhập';
    }
